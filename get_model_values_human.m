...
function [beta, t, SE, CI, all_coords, translated_coords, varargout] = get_model_values(MRI_directory, subjid, modeltype, contrast, ROI_file, varargin)
...
if length(varargin) > 0
    ...
    ROI_epi_subtract = readnifti(subtraction_file);
    ROI_s = ROI_epi_subtract > 0;
    [x_s, y_s, z_s] = ind2sub(size(ROI_epi_subtract), find(ROI_s == 1));
    subtraction_coordinates = [x_s, y_s, z_s];
    [C, ia, ib] = intersect(ROI_coordinates, subtraction_coordinates, 'rows');
    ROI_coordinates(ia, :) = [];
end
...
cd(fullfile(MRI_directory, num2str(subjid), 'models', modeltype))
load('SPM.mat');
...
%   coords = [ROI_x(i); ROI_y(i); ROI_z(i)];
    ...
    %[currbeta, currSE, currCI] = extractSPMData(xSPM,SPM,trans_coords);
