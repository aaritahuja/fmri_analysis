function art_movie_sub(Action,files,f3)
...
% art_movie_sub.m by CG & TMD to not create it's own figure Nov 2012.
...
% GET THE USER'S INPUTS
 % For calling in line with arguments
...
%     end
    ...
    Orient= 1; % spm_input('Select slice orientation',...
    ...
    scansel = 1:length(F); %spm_input([num2str(nFsize) ' vols. Select range, e.g. 10:99'],1);  %array of integers
%     if slnum > 9   % 9 is for the HiResolution case.
     CloseUp = 0; % all images,   %%%spm_input('All Slices or ~20 close-up slices',1, ' All | Close-up', [ 0 1 ] );
...
%          slicesel = max(1,slicesel - 12);  %  slicesel is now the first slice.
    ...
    typesel = 2; %%% contrasts... spm_input('Select Data Magnification',1,'m', ...
...
%         [ 1 2 3 ], 2);
    ...
    if typesel > 1   % For contrast modes, ask for a reference image
        refsel = 1;  %%% automatic.   spm_input('Choose a Reference Image',1,'m', ...
...
%             fref = deblank(Frefer);
    ...
    smode= 1; %spm_input('Select viewing mode',1,'m', ...
...
figure(f3);   % Standard Matlab window has zoom enlargement, etc.
...
%----------------------------------------------------------
  % Create the movie or slider figure.
   ha = f3;
  figure(ha);
colormap(cmap);
