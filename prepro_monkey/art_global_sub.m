function art_global_sub(Images,f2,RealignmentFile,HeadMaskType, RepairType)
...
% art_global_sub.m by CG & TMD to not create it's own figure Nov 2012.
% Modified by TMD 11/10/15 to include SPM12 in the list of SPM versions
...
%     end
    ...
    if strcmp(spm_ver,'spm5')
        for i = 1:num_sess
            P{i} = Images %spm_select(Inf,'image',['Select data images for session'  num2str(i) ':']);
        end
    end
...
elseif realignfile == 0
	...
	legend('x mvmt', 'y mvmt', 'z mvmt', 'Best');
	h = gca;
	set(h,'Ygrid','on');
end 
