...
# get initial time
t0=$(date +%s)
# load modules for this batch we also need to load fsl
module load spm
module load matlab
module load fsl
...
export PATH_IN_T1=$PATH_IN/$MONKEY/$SESS/anatomy/t1_mprage_mean
...
export PATH_OUT=/gpfs/data/tdesroch/monkey/$MONKEY/$SESS/
...
rsync -az $PATH_IN_T1/ $PATH_TMP_ANAT/t1_mprage_mean
...
cd $PATH_TMP_SCRIPTS
matlab-threaded -nodisplay -r "td_prepro_T1normbatch('$PATH_TMP', '$PATH_TMP_SESS', '$PATH_TMP_ANAT'), exit"
